...
struct FinanceTimelineProvider: AppIntentTimelineProvider {
    ...
    private var sharedDefaults: UserDefaults {
        UserDefaults(suiteName: KeychainHelper.sharedAccessGroup) ?? .standard
    }
    ...
    private func cacheSummary(_ summary: FinanceSummary) {
        ...
        sharedDefaults.set(data, forKey: Self.cacheKey)
    }
    ...
    private func loadCached(applyingLimit spendLimit: Double) -> FinanceSummary? {
        guard let data = sharedDefaults.data(forKey: Self.cacheKey) else { return nil }
